...
// SDL2 libraries are now linked via CMake
// #pragma comment(lib, "SDL2.lib")
// #pragma comment(lib, "SDL2main.lib")

// OpenGL libraries are now linked via CMake
// #pragma comment(lib, "opengl32.lib")
// #pragma comment(lib, "glu32.lib")

// Windows libraries are now linked via CMake
  ...
  bool toggleFullscreen() {
    ...
    if (!fullscreen_) {
      printf(__FUNCTION__ ": to fullscreen %dx%d display %d\n", targetWidth,
             targetHeight, d);
      window_ = SDL_CreateWindow("boxplorer2", rect_[d].x, rect_[d].y,
...
float getFPS(void) {
  ...
  if (lastFrameTime - lastfps > 5000) {
    // Once per 5 seconds.
#if defined(_WIN32) && defined(HAVE_OCULUS_SDK)
...
int main(int argc, char **argv) {
  ...
  if (setupDirectories(configFile) && config.loadConfig(BaseFile, &defines)) {
    ...
    if (GetOpenFileName(&opf) && setupDirectories(opf.lpstrFile) &&
  ...
  while (!done) {
    ...
    if (!rendering) {
      ...
      if (!config.disable_de) {
        ...
        if (de_func || de_func_64) {
          ...
          de =
              de_func_64 ? GLSL::abs(de_func_64(pos)) : GLSL::abs(de_func(pos));
    ...
    sprintf(caption, "%s %.2ffps %5lu [%.3lf %.3lf %.3lf] %dms",
            ...
            camera.pos()[2], getLastFrameDuration());
